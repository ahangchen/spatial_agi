...
- **Python 环境**：**必须使用 miniconda3 的 base 环境**。所有 `zhihu` 命令必须通过 `~/miniconda3/bin/zhihu` 执行，或先激活 base 环境（`source ~/miniconda3/bin/activate`）。
- **已安装**：`zhihu` 已安装在 miniconda3 base 环境中（`~/miniconda3/bin/zhihu`）。
...
- **本机安装状态**：✅ 已安装在 miniconda3 base 环境中（`~/miniconda3/bin/zhihu`，版本 0.2.3）。
- **更新**：`~/miniconda3/bin/pip install -U pyzhihu-cli`。
- **版本**：`~/miniconda3/bin/zhihu --version` 查看当前版本。

---

## Agent 规则

1. **环境要求**：所有 `zhihu` 命令必须使用完整路径 `~/miniconda3/bin/zhihu`，或在执行前先 `source ~/miniconda3/bin/activate`。
2. **诉求 → 命令**：按下表映射。
...
~/miniconda3/bin/zhihu search "Python" --json
~/miniconda3/bin/zhihu hot --limit 10
~/miniconda3/bin/zhihu question 12345678
~/miniconda3/bin/zhihu user someone
~/miniconda3/bin/zhihu vote 87654321
~/miniconda3/bin/zhihu pin "标题" -c "<p>正文</p>"
~/miniconda3/bin/zhihu delete-pin 98765432 -y
~/miniconda3/bin/zhihu notifications -l 10
~/miniconda3/bin/zhihu logout
```
