...
**2. VGGT-SLAM 1.0**
- **核心技术**: SL(4)流形优化（15自由度）
- **解决问题**: 投影多义性（剪切、拉伸、透视畸变）
- **优势**: 无需相机内参，未标定单目SLAM
- **局限**: 平面场景退化，纯旋转漂移
...
- **创新点**: 
  ...
  - 帧间边：强制重叠一致性
  - 第22层注意力"聚光灯效应"验证闭环
- **性能**: 位姿误差降低23%，实时运行（Jetson Thor）

**4. InfiniteVGGT**
- **核心技术**: 滚动记忆（Rolling Memory）
- **解决问题**: 显存溢出（OOM），长期漂移
- **创新点**:
  - 不可变锚点：全局参考坐标系
  - 键空间多样性代理：token重要性评估
...
**来源**: VGGT SLAM系列论文分析（4篇）
**标签**: `#spatial-agi` `#slam` `#evolution` `#technical-roadmap`
