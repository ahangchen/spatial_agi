...
- 🚀 **dVLA-RL 首次将 RL 应用于离散扩散 VLA**: 将去噪过程建模为 MDP,提出轨迹级概率建模替代不可行的边缘动作概率,优雅绕过组合爆炸。LIBERO 99.7% SOTA + RoboTwin 2.0 30.6% 绝对提升(61.4% → 92.0%)。混合步调度策略实现任务自适应计算分配--**简单任务1步、复杂任务4步**,为空间动作生成开辟第三种范式。

### 📊 一句话总结

今天的五篇论文从系统架构(HoloAgent-0 的闭环执行)、空间效率(Pocket-SLAM 的内存剪枝)、世界模型协同(NavWM 的三任务统一)、持续学习(ReGen 的生成式回放)、动作生成范式(dVLA-RL 的离散扩散+RL后训练)五个维度,共同指向:**Spatial AGI 正在从"组件突破"进入"系统闭环"阶段--不再是单个算法的进步,而是如何将感知、记忆、规划、执行组织成持续运行的闭环系统,并通过 RL 后训练不断逼近性能上限**。

### 🔗 延续性

**昨日(06-22)→今日(06-27)**: 昨天确立了"接口革命"--μ0 的 3D trace、PAIWorld 的跨视图通信、GASP 的几何先验注入、Stream3D-VLM 的流式控制各自找到了"正确接口"。今天将这一视角推进到系统层面:**正确的接口需要被组织在闭环系统中才能发挥效用**。
...
- dVLA-RL 的轨迹级概率建模为 VLA 的 RL 后训练提供了新范式,与 Embodied-R1.5 的"推理内部化"互补--前者优化动作生成质量,后者优化空间推理能力

**今日→明日**: 五篇论文各自解决了闭环系统的一个方面(架构、效率、协同、记忆、动作优化),但组件间的集成尚未探索。

### 💡 本质思考:如何达成通用空间智能

#### 1. 核心能力的本质是什么?

今天揭示了一个深层规律:**Spatial AGI 的核心能力不是感知、规划或行动中的任何一项,而是"闭环"--持续的环境交互、信念更新和自我修正**。HoloAgent-0 的 AgentOS 是"闭环组织器",NavWM 的世界模型是"闭环规划器",ReGen 的 WAM 是"闭环学习器",dVLA-RL 的轨迹级 RL 是"闭环优化器"--通过 RL 后训练不断逼近性能上限。Spatial AGI 的本质不是任何单一能力的极致,而是**能力的组织方式与持续优化**。
...
**三阶段演进**:(1) 闭环系统集成+RL后训练(当前-2027中):组合今天的五项突破为统一系统,dVLA-RL 式的 RL 后训练成为标准流程;(2) 长程闭环突破(2027中-2028):解决递归退化,发展跨天/跨场景学习;(3) Spatial AGI 部署(2028+):真实环境持续运行,多embodiment协作。
...
### 昨日重点(2026-06-22)
昨天聚焦"接口革命"--μ0(3D trace)、PAIWorld(跨视图通信)、GASP(几何先验注入)、Embodied-R1.5(坐标推理)、Stream3D-VLM(流式控制)各自找到"正确接口"。核心转折点:接口标准化。

### 今日进展

1. **从组件接口到系统接口**: HoloAgent-0 的 Typed Skill Interface 是 μ0 trace 接口在系统级的延伸--trace 是感知-行动之间的抽象,Typed Skill 是规划-执行之间的抽象
...
5. **从 SFT 到 RL 后训练**: dVLA-RL 将 Embodied-R1.5 的"推理内部化"推进到"动作优化外部化"--通过 RL 后训练在离散扩散框架下逼近最优策略,30.6% 的绝对提升证明 SFT 远非性能上限

### 演进路径

```mermaid
graph LR
    A["06-22: μ0<br/>3D trace 通用接口"] --> B["06-27: HoloAgent-0<br/>Typed Skill 系统接口"]
    ...
    N["06-22: Embodied-R1.5<br/>推理内部化"] --> O["06-27: dVLA-RL<br/>RL后训练动作优化"]
    B --> J
    D --> J
    F --> J
    H --> J
...
- **机构**: 上海交通大学、清华大学、百度、地平线机器人、上海AI Lab
- **核心**: 首次将 RL 应用于离散扩散 VLA,提出轨迹级概率建模替代不可行的边缘动作概率,将去噪过程建模为 MDP。混合步调度策略根据任务难度自适应分配去噪步数(1-4步)。
- **关键数据**: LIBERO 99.7% SOTA; RoboTwin 2.0 30.6% 绝对提升(61.4% → 92.0%,VLA 方法第一); Hybrid 推理延迟 387ms
- **与 Spatial AGI**: 为空间动作生成提供第三种范式(离散扩散+RL),统一多模态 token 空间,混合步调度实现任务自适应计算分配
...
| dVLA-RL | 精度+泛化(RL后训练+30.6%) | 效率代价(多步去噪182-728ms) |

**关键发现**: 没有任何单一方法同时优化三个维度。Pocket-SLAM 最接近"免费午餐"(效率大幅提升,精度不退化),但它是静态场景方法。dVLA-RL 通过 Hybrid 步调度在精度和效率间找到自适应平衡点--简单任务 1 步(182ms)、复杂任务 4 步(729ms)。真正的 Spatial AGI 需要在三个维度间动态权衡--根据当前任务需求和资源约束调整分配。
...
- ✅ dVLA-RL 的统一离散 token 空间:视觉、语言、动作统一到同一离散 token 空间,使得 RL 可以在统一的概率框架下优化多模态生成路径。轨迹级概率建模统一了不同去噪步数(1/2/4步)的优化目标

**对 Spatial AGI 的启发**:
昨天 PAIWorld 证明了通信×几何的超加性效应(2.64 >> 1.65),今天 NavWM 在更广义的层面证明了感知×生成×控制的超加性,dVLA-RL 在动作生成层面证明了离散统一空间的 RL 优化可行性。**统一架构的协同效应不是偶发现象,而是世界模型的普遍规律**--因为感知、生成和控制本质上都依赖于对同一物理世界的理解,共享这种理解必然比各自独立学习更高效。

这进一步强化了 WAM 作为 Spatial AGI 核心构件的论点:WAM 天然地将感知(世界推理)、控制(动作预测)和生成(视觉预见)统一在一个模型中,享受跨任务的协同效应。而 dVLA-RL 的统一 token 空间则提供了一种实现这种统一的具体技术路径。

### 见解8: RL 后训练是解锁空间策略性能上限的关键
**来源**: dVLA-RL + ReGen + NavWM

- ✅ dVLA-RL 证明 RL 后训练为离散扩散 VLA 带来 30.6% 绝对提升(RoboTwin 2.0),SFT 61.4% → RL 92.0%--仅靠模仿学习训练的空间策略远未达到性能上限
- ✅ ReGen 的递归生成式回放本质上也是一种"后训练"--在持续学习阶段通过生成数据不断微调策略,遗忘降低 50%+
- ✅ NavWM 的 Exposure Bias 微调(Stage 2)同样是一种后训练策略--在 Teacher Forcing 之后用模型自身预测轨迹微调 CDiT,纠正曝光偏差

**对 Spatial AGI 的启发**:
Spatial AGI 系统应该将 RL 后训练作为标准流程。SFT 提供基础策略,RL 通过环境反馈不断优化--这与人类"先模仿学习,后试错改进"的学习过程高度一致。dVLA-RL 的轨迹级概率建模为这一方向提供了可推广的数学框架:**优化实际执行路径的概率,而非终端输出的概率**。
...
```mermaid
graph LR
    A["06-22: μ0<br/>3D trace 接口"] --> B["06-27: HoloAgent-0<br/>Typed Skill 系统接口"]
...
graph LR
    ...
    P["06-22: VLA SFT 范式"] --> Q["06-27: dVLA-RL<br/>离散扩散+RL后训练"]

    C --> R["06-27 趋同结论<br/>多层级闭环系统集成+RL后训练"]
    F --> R
    I --> R
    L --> R
    O --> R
...
| **L7 动态推理** | 超加性协同 + Trace多模态预测 | NavWM三任务协同(1+1+1>3) + ReGen生成式回放 + HoloAgent闭环修正 + dVLA-RL轨迹级概率建模 | 协同效应从两组件扩展到三任务;闭环修正能力成为核心;新增去噪轨迹MDP优化 |
| **L8 空间行动** | Trace→Action Expert + DiT-B动作头 | HoloAgent-0 Typed Skill Interface + ReGen WAM(动作+观察联合) + dVLA-RL离散扩散动作生成 | 新增物理技能"API化"范式;WAM的多重身份(策略+模拟器+记忆);新增离散扩散+RL第三种动作生成范式 |
...
| **L10 训练范式** | 无动作标签预训练 + 15B token+多任务RL + 训练时注入推理时移除 | 两阶段联合训练(teacher forcing+exposure bias微调) + ReGen递归回放 + 即时剪枝 + dVLA-RL轨迹级PPO后训练 | 新增Exposure Bias微调;新增递归生成式回放;新增任务感知的表示压缩;新增离散扩散VLA的RL后训练标准流程 |
...
- **当前最佳方案**: NavWM(三任务统一训练)+ ReGen(持续学习回放)+ dVLA-RL(RL后训练优化)
  - NavWM 证明感知×生成×控制的协同效应(ATE 0.254 vs 0.338)
  - ReGen 证明 WAM 可以通过生成回放保留旧知识(遗忘降低50%+)
  - dVLA-RL 证明 RL 后训练可将 SFT 策略性能提升 30.6%(61.4% → 92.0%)
  - 三者共同指向 WAM 作为 Spatial AGI 核心构件
- **关键挑战**: NavWM 的 44% 零样本成功率仍低;ReGen 的递归退化(PSNR 随阶段下降);想象-执行鸿沟(83% vs 42%)
- **预计成熟**: 2027 年中

#### 阶段3: 闭环执行系统集成(稳步推进中 50%)
- **核心问题**: 如何将感知、记忆、规划和执行组织成持续运行的闭环系统?
- **当前最佳方案**: HoloAgent-0 的 AgentOS 三层耦合架构 + dVLA-RL 的 RL 后训练
  - AgentOS(推理+规划)+ Memory(HMSG+时间记忆)+ Skill(Typed Interface)
  - 五种恢复策略处理物理不确定性
  - ROS2 Command/Status Bus 保持模块化
  - dVLA-RL 的轨迹级 RL 为动作技能提供性能提升路径(SFT→RL 标准流程)
- **关键挑战**: 定量评估仅覆盖导航和映射,操作和协调仅定性演示;LLM 依赖的隐藏成本;安全保证不明确;dVLA-RL 仅在仿真验证,真实世界迁移未知
- **竞争方案**: 端到端 VLA(π0.5 等)--更简单但缺乏可解释的闭环
- **预计成熟**: 2027 年中

#### 阶段4: 多层级闭环融合(展望中 25%)
- **核心问题**: 执行级、规划级、学习级、记忆级的闭环如何协调?
- **愿景**: 一个系统--HoloAgent 的执行闭环 + NavWM 的规划闭环 + ReGen 的学习闭环 + Pocket-SLAM 的记忆闭环 + dVLA-RL 的优化闭环
- **关键挑战**: 各闭环的时间尺度不同(毫秒级执行→分钟级规划→小时级学习→天级记忆);计算资源分配;安全保证;RL 后训练的仿真-现实迁移
- **预计成熟**: 2028+
...
4. **dVLA-RL 轨迹级 RL × NavWM 世界模型的"想象-执行"闭环**
   - 问题: dVLA-RL 通过 RL 后训练优化动作生成,NavWM 通过世界模型预见评估候选轨迹。能否将 NavWM 的视觉预见作为 dVLA-RL 的奖励信号?dVLA-RL 的去噪轨迹能否用 NavWM 的世界模型来"预演"和筛选?
   - 候选方案: 双模型协作--dVLA-RL 生成候选动作 → NavWM 预见未来状态 → 基于预见质量筛选或奖励 → dVLA-RL 轨迹级 PPO 优化
   - 缺口: 两个模型的动作空间表示不同(离散 token vs 连续);推理延迟叠加;奖励信号设计

5. **Pocket-SLAM 语义感知剪枝**
...
6. **NavWM Latent World Tokens × GASP Correspondence 先验**
...
7. **HoloAgent-0 的 "View Layer" 独特价值**
...
8. **Typed Skill Interface × μ0 Trace 接口的统一**
   - 问题: HoloAgent-0 的 Typed Skill Interface(系统级)和 μ0 的 3D trace(组件级)分别代表了不同抽象层级的接口设计。能否构建统一的接口层次--trace 作为"底层运动接口",Typed Skill 作为"高层任务接口"?
...
12. **dVLA-RL 离散动作 token 的精度极限与 3D 空间表示扩展**
    - 问题: dVLA-RL 将连续动作量化为离散 token,这在需要亚毫米级精度的任务中可能成为瓶颈。同时,dVLA-RL 依赖 2D 图像输入,缺乏显式 3D 空间表示。能否在离散 token 空间中引入 3D 几何信息(如点云 token、深度 token)来提升空间推理能力?
    - 候选方案: 扩展统一 token 空间--视觉 token + 语言 token + 动作 token + 几何 token(点云/深度离散化),在统一的离散扩散框架下联合去噪生成
    - 缺口: 几何 token 的离散化方案;token 空间维度的计算开销;与 HMSG 层次化表示的接口
...
- ✅ 空间表示:层次化场景图(HMSG)、3DGS表示与剪枝、B-spline轨迹、Latent World Tokens、统一离散token空间(dVLA-RL)
...
- ✅ VLA 后训练:离散扩散VLA的轨迹级RL优化(dVLA-RL)、Hybrid步调度自适应计算分配
...
- ⚠️ 实时性:NavWM未报告FPS,HoloAgent-0依赖cloud LLM,ReGen生成开销,dVLA-RL多步去噪延迟182-728ms
- ⚠️ 端到端评估:HoloAgent-0缺乏端到端任务完成率评估
- ⚠️ 仿真-现实迁移:dVLA-RL仅在仿真验证,真实世界效果未知
- ⚠️ 离散动作精度:dVLA-RL的连续动作量化误差对精密任务的影响未评估
...
| 方法 | LIBERO 平均(%) | RoboTwin 2.0(%) | 范式 |
|------|--------------|----------------|------|
| OpenVLA | 76.5 | - | 自回归 SFT |
| π0 | 94.2 | 52.1 | 连续扩散 SFT |
| π0.5 | 96.9 | 74.9 | 连续扩散 SFT |
| OpenVLA-OFT | 97.1 | 34.2 | 自回归 SFT+OFT |
...
### 对比1: 空间记忆范式--HoloAgent-0 vs ReGen vs NavWM vs dVLA-RL
...
| **适用范围** | 长期环境地图 | 技能/任务记忆 | 在线场景理解 | 动作策略记忆(权重中) |

**关键发现**: 四种记忆/表示范式不是竞争关系,而是互补关系。HoloAgent-0 的 HMSG 回答"环境是什么样的",ReGen 回答"我以前学过什么技能",NavWM 回答"当前场景有什么结构信息",dVLA-RL 的统一 token 空间回答"如何将感知-语言-动作统一表示"。一个完整的 Spatial AGI 系统需要全部四种。

### 对比2: 闭环系统设计--HoloAgent-0 vs NavWM vs ReGen vs dVLA-RL

| 维度 | HoloAgent-0 AgentOS | NavWM 闭环规划 | ReGen 持续学习 | dVLA-RL 轨迹级RL |
|------|---------------------|---------------|---------------|-----------------|
| **闭环范围** | 单任务执行 | 轨迹选择 | 任务序列学习 | SFT→RL后训练 |
| **时间尺度** | 秒-分钟 | 毫秒-秒 | 小时-天 | 训练时(离线) |
| **反馈来源** | 技能状态流(结构化) | 视觉预见(生成式) | 奖励预测(标量) | 环境奖励(标量) |
| **修正能力** | 五种恢复策略 | K候选评估选择 | 递归回放补偿 | PPO信任区域约束 |
| **错误检测** | 验证后置条件 | 未来观察匹配度 | PSNR质量监控 | 策略优势函数估计 |

**系统级洞察**: 当前每种闭环只在单一时间尺度运行。真正的 Spatial AGI 需要嵌套闭环--执行级闭环(毫秒)嵌入规划级闭环(秒),再嵌入学习级闭环(小时),再嵌入记忆级闭环(天),再嵌入优化级闭环(训练时)。这种嵌套闭环的设计是一个开放挑战。

### 对比3: 效率策略--Pocket-SLAM vs NavWM vs HoloAgent-0 vs dVLA-RL

| 策略 | Pocket-SLAM | NavWM | HoloAgent-0 | dVLA-RL |
|------|------------|-------|-------------|---------|
| **效率优化** | 内存削减65%+ | 参数共享(1.5B统一) | 模块化(可替换组件) | Hybrid步调度(1-4步自适应) |
| **效率代价** | 微小精度损失 | 训练成本(8×A100) | 系统复杂度 | 多步去噪延迟(182-728ms) |
| **部署门槛** | NVIDIA A6000 48GB | 未报告推理硬件 | 需cloud LLM + ROS2 | 仿真环境训练 |
| **关键洞察** | 删除时机=删除策略重要性 | 统一比专用更高效 | 接口设计比算法更重要 | 任务复杂度驱动计算分配 |

### 对比4: VLA 动作生成范式--dVLA-RL vs 其他 VLA 方法

| 维度 | dVLA-RL (离散扩散+RL) | SimpleVLA-RL (自回归+RL) | π0/π0.5 (连续扩散+SFT) | ReGen/NavWM (WAM) |
|------|----------------------|--------------------------|----------------------|-------------------|
| **动作生成** | K步去噪迭代精炼 | 逐token自回归 | 连续流匹配采样 | 世界模型联合预测 |
| **RL可行性** | 轨迹级概率可处理 | 天然可处理(token级) | 需SDE近似 | 非RL(BC+生成回放) |
| **多步推理** | ✅ 1-4步自适应 | ❌ 单步 | ✅ 多步扩散 | ✅ 多步生成 |
| **统一空间** | ✅ 视觉+语言+动作统一token | ❌ 动作单独空间 | ❌ 动作连续空间 | 部分(观察+动作) |
| **LIBERO** | **99.7%** | 99.1% | 94.2%/96.9% | - |
| **RoboTwin** | **92.0%** | 68.7% | 52.1%/74.9% | 84.6%/96.6% |
| **核心优势** | 统一空间+自适应计算 | 最简单的RL框架 | 大量真实验证 | 前瞻预测能力 |
...
$$\log \pi_\theta(\tau \mid s_t) = \sum_{k=K}^{1} \left[ \log P_{\text{unmask}}(U_k \mid p_\theta, k) + \sum_{i \in U_k} \log p_\theta(x_{k-1}^i \mid x_k, s_t) \right]$$

**可处理代理目标**(隐式调度器优化):

$$\mathcal{L}^{\text{dVLA-RL}}(\theta) = \mathbb{E}_\tau\left[ \sum_{k=K}^{1} \sum_{i \in M_k} \log p_\theta(x_{k-1}^i \mid x_k, s_t) \cdot A_t \right]$$

其中 $M_k \subseteq U_k$ 是从 [MASK] 转换为具体 token ID 的子集,$A_t$ 为 GAE 估计的优势函数。

### dVLA-RL: Hybrid 步调度

| 任务复杂度 | 去噪步数 | 推理延迟 | 训练时间 |
|----------|---------|---------|---------|
| 简单(SFT强) | 1 | 182ms | 259s/epoch |
| 中等 | 2 | 364ms | 452s/epoch |
| 复杂(SFT弱) | 4 | 729ms | 846s/epoch |
| **Hybrid** | **1/2/4** | **387ms** | **608s/epoch** |
...
传统机器人范式是"感知→规划→行动"(Sense-Plan-Act)。今天的五篇论文暗示着新范式的到来:

**"想象→执行→回忆→优化"(Imagine-Act-Remember-Optimize)**
...
4. **优化(Optimize)**: dVLA-RL 的轨迹级 RL--通过 PPO 在去噪轨迹上优化策略概率,逼近性能上限

这个新范式与传统范式的关键区别:
- **想象替代了规划**:不是符号推理链,而是通过世界模拟来选择行动
- **执行包含了修正**:不是开环执行,而是持续的监控-验证-恢复
- **回忆替代了存储**:不是检索旧数据,而是通过生成重建过去
- **优化替代了固化**:不是训练后固定,而是通过 RL 后训练持续提升
...
| 训练时 | 策略优化 | dVLA-RL 轨迹级PPO | 在线RL+持续适应 |
| 周级 | 知识固化 | 未解决 | 跨环境迁移 |
...
- ✅ 核心见解: 8 个(闭环修正、记忆二元性、任务感知、WAM多重身份、效率-精度-泛化三角、语言索引、协同效应普遍性、RL后训练)
- ✅ 技术栈演进对比表: 10 层完整覆盖
- ✅ 主线技术路径: 5 个阶段
- ✅ 待探索议题: 12 个(分3个优先级)
- ✅ 本质思考: 3 个子问题
- ✅ 知识缺口分析: 三分类(35%/40%/25%)
- ✅ 论文间深度对比: 4 组对比(含 VLA 动作生成范式对比)
- ✅ 知识缺口分析: 三分类(35%/40%/25%)
...
### 局限5: RL 后训练的仿真-现实鸿沟
 dVLA-RL 在 LIBERO 和 RoboTwin 2.0 仿真环境中取得优异成绩，但真实世界验证缺失。仿真环境的奖励设计可能与真实世界存在系统偏差，目RL策略在 distribution shift 下的鲁棒性未知。此外，离散动作 token 的量化误差在需要亚毫米级精度的真实操作任务中可能成为瓶颈。

**风险**: 仿真训练的 RL 策略可能无法直接迁移到真实机器人。需要 sim2real 技术和真实验证。
...
1. **Spatial AGI 正在进入“系统闭环”阶段**:不再是单个算法的突破,而是如何将感知、记忆、规划、执行组织成持续运行的闭环系统,并通过 RL 后训练逼近性能上限。HoloAgent-0 的三层耦合架构、NavWM 的三任务协同、ReGen 的生成式回放、Pocket-SLAM 的任务感知剪枝、dVLA-RL 的轨迹级 RL 后训练,分别从不同维度贡献了系统闭环的关键组件。

2. **WAM 是 Spatial AGI 的核心构件,RL 后训练是解锁其潜力的关键**:NavWM 和 ReGen 从不同角度证明了 WAM 的“多重身份”--它是策略、模拟器、记忆系统和规划器的统一体。dVLA-RL 进一步证明,通过 RL 后训练可将 SFT 策略提升 30.6%,说明 SPT 远非性能上限。统一架构的协同效应(1+1+1 > 3)使得持续学习无需额外存储成为可能。

3. **空间记忆与动作生成需要混合架构**:显式存储(HMSG)提供精确性,生成回放(ReGen)提供灵活性,剪枝压缩(Pocket-SLAM)控制增长,隐式编码(Latent Tokens)提供效率,离散扩散+RL(dVLA-RL)提供动作优化。五种机制的正确组合--而非任何单一机制--才能支撑长时序的空间智能。

### 对明天研究的期待

- HoloAgent-0 的系统架构 + NavWM 的世界模型能否组合?
- ReGen 的生成式记忆 + Pocket-SLAM 的剪枝策略能否统一?
- dVLA-RL 的轨迹级 RL + NavWM 的世界模型预见能否形成“想象-优化”闭环?
- dVLA-RL 的统一离散 token 空间 + HoloAgent-0 的 HMSG 能否实现 3D 空间感知与动作生成的统一?
...
*今天,我们看到了 Spatial AGI 从“组件时代”向“系统时代”的转折。接口已找到,记忆已成形,闭环已建立,RL 后训练开辟了新的优化空间。下一步是集成与持续优化。*
...
| dVLA | Discrete Diffusion Vision-Language-Action | dVLA-RL |
| MDP | Markov Decision Process (去噪过程建模) | dVLA-RL |
| PPO | Proximal Policy Optimization | dVLA-RL |
| GAE | Generalized Advantage Estimation | dVLA-RL |
